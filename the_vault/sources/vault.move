module the_vault::vault{
    ...
    struct VaultSignerCapability has key {
        cap: account:VaultSignerCapability
    }
    fun init_module(resource_amount: &signer){
        let resource_account_address = signer::address_of(resource_account);

        let(vault_signer, vault_signer_cap)=account::create_resource_account(resource_account_address,b"Vault Address" );
